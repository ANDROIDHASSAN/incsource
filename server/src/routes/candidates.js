...
candidatesRouter.get('/', asyncHandler(async (req, res) => {
  res.json(await store.listCandidates(req.query));
}));

// Header stats.
candidatesRouter.get('/stats', asyncHandler(async (_req, res) => {
...
candidatesRouter.post('/rescore', rateLimit({ windowMs: 60_000, max: 6 }), asyncHandler(async (_req, res) => {
  res.json(await store.rescoreAll((c) => scoreActiveIntent(c)));
}));

// Analytics for the dashboard (respects filters).
candidatesRouter.get('/analytics', asyncHandler(async (req, res) => {
  res.json(await store.analytics(req.query));
}));

// Filter-dropdown facets.
candidatesRouter.get('/facets', asyncHandler(async (_req, res) => {
  res.json(await store.facets());
}));
...
candidatesRouter.get('/export', asyncHandler(async (req, res) => {
  const { candidates } = await store.listCandidates({ ...req.query, limit: 500, offset: 0 });
...
candidatesRouter.post('/:id/outreach', asyncHandler(async (req, res) => {
  const c = await store.recordOutreach(req.params.id, { channel: req.body?.channel || 'email', subject: req.body?.subject || '' });
  if (!c) return res.status(404).json({ error: 'Not found' });
  res.json(c);
}));

// Find a verified email for ONE candidate (enrichment).
candidatesRouter.post('/:id/enrich', rateLimit({ windowMs: 60_000, max: 30 }), asyncHandler(async (req, res) => {
  const c = await store.getCandidate(req.params.id);
  if (!c) return res.status(404).json({ error: 'Not found' });
  const result = await enrichContacts([c]);
  if (result.error) return res.status(502).json({ error: result.error });
  const updated = await store.setContact(c.id, { email: c.email });
  res.json({ candidate: updated, found: { email: Boolean(updated.email) } });
}));

// Bulk-enrich selected candidates (capped for spend).
candidatesRouter.post('/enrich', rateLimit({ windowMs: 60_000, max: 6 }), asyncHandler(async (req, res) => {
  ...
  if (result.error) return res.status(502).json({ error: result.error });
  await Promise.all(list.map((c) => store.setContact(c.id, { email: c.email })));
  res.json(result);
}));

// Bulk update (status/star/tags) or delete. Body: { ids: [], patch: {} } or { ids: [], delete: true }.
candidatesRouter.post('/bulk', asyncHandler(async (req, res) => {
  const ids = Array.isArray(req.body?.ids) ? req.body.ids.slice(0, 1000) : [];
  if (!ids.length) return res.status(400).json({ error: 'No ids provided' });
  if (req.body.delete) return res.json({ deleted: await store.bulkDelete(ids) });
  const patch = sanitizePatch(req.body?.patch || {});
  res.json({ updated: await store.bulkUpdate(ids, patch) });
}));
...
candidatesRouter.patch('/:id', asyncHandler(async (req, res) => {
  const c = await store.updateCandidate(req.params.id, sanitizePatch(req.body || {}));
  if (!c) return res.status(404).json({ error: 'Not found' });
  res.json(c);
}));

candidatesRouter.delete('/:id', asyncHandler(async (req, res) => {
